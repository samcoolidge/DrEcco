(function(window){'use strict';function createInstance(func,args){function F(){return func.apply(this,args)}
F.prototype=func.prototype;return new F()}
function minErr(module,ErrorConstructor){ErrorConstructor=ErrorConstructor||Error;return function(){var SKIP_INDEXES=2;var templateArgs=arguments,code=templateArgs[0],message='['+(module?module+':':'')+code+'] ',template=templateArgs[1],paramPrefix,i;message+=template.replace(/\{\d+\}/g,function(match){var index=+match.slice(1,-1),shiftedIndex=index+SKIP_INDEXES;if(shiftedIndex<templateArgs.length){return toDebugString(templateArgs[shiftedIndex])}
return match});message+='\nhttp://errors.angularjs.org/1.5.5/'+(module?module+'/':'')+code;for(i=SKIP_INDEXES,paramPrefix='?';i<templateArgs.length;i++,paramPrefix='&'){message+=paramPrefix+'p'+(i-SKIP_INDEXES)+'='+encodeURIComponent(toDebugString(templateArgs[i]))}
...
HashMap.prototype={put:function(key,value){this[hashKey(key,this.nextUid)]=value},get:function(key){return this[hashKey(key,this.nextUid)]},remove:function(key){var value=this[key=hashKey(key,this.nextUid)];delete this[key];return value}};var $$HashMapProvider=[function(){this.$get=[function(){return HashMap}]}];var ARROW_ARG=/^([^\(]+?)=>/;var FN_ARGS=/^[^\(]*\(\s*([^\)]*)\)/m;var FN_ARG_SPLIT=/,/;var FN_ARG=/^\s*(_?)(\S+?)\1\s*$/;var STRIP_COMMENTS=/((\/\/.*$)|(\/\*[\s\S]*?\*\/))/mg;var $injectorMinErr=minErr('$injector');function extractArgs(fn){var fnText=Function.prototype.toString.call(fn).replace(STRIP_COMMENTS,''),args=fnText.match(ARROW_ARG)||fnText.match(FN_ARGS);return args}
...
if(!isClass(fn)){return fn.apply(self,args)}else{return createInstance(fn,args)}}
function instantiate(Type,locals,serviceName){var ctor=(isArray(Type)?Type[Type.length-1]:Type);var args=injectionArgs(Type,locals,serviceName);return createInstance(ctor,args)}
...
if(optional&&!attrs[attrName])break;parentGet=$parse(attrs[attrName]);destination[scopeName]=parentGet(scope);initialChanges[scopeName]=new SimpleChange(_UNINITIALIZED_VALUE,destination[scopeName]);removeWatch=scope.$watch(parentGet,function parentValueWatchAction(newValue,oldValue){if(newValue===oldValue){oldValue=destination[scopeName]}
...
$provide.value("$locale",{"DATETIME_FORMATS":{"AMPMS":["AM","PM"],"DAY":["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],"ERANAMES":["Before Christ","Anno Domini"],"ERAS":["BC","AD"],"FIRSTDAYOFWEEK":6,"MONTH":["January","February","March","April","May","June","July","August","September","October","November","December"],"SHORTDAY":["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],"SHORTMONTH":["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],"STANDALONEMONTH":["January","February","March","April","May","June","July","August","September","October","November","December"],"WEEKENDRANGE":[5,6],"fullDate":"EEEE, MMMM d, y","longDate":"MMMM d, y","medium":"MMM d, y h:mm:ss a","mediumDate":"MMM d, y","mediumTime":"h:mm:ss a","short":"M/d/yy h:mm a","shortDate":"M/d/yy","shortTime":"h:mm a"},"NUMBER_FORMATS":{"CURRENCY_SYM":"$","DECIMAL_SEP":".","GROUP_SEP":",","PATTERNS":[{"gSize":3,"lgSize":3,"maxFrac":3,"minFrac":0,"minInt":1,"negPre":"-","negSuf":"","posPre":"","posSuf":""},{"gSize":3,"lgSize":3,"maxFrac":2,"minFrac":2,"minInt":1,"negPre":"-\u00a4","negSuf":"","posPre":"\u00a4","posSuf":""}]},"id":"en-us","localeID":"en_US","pluralCat":function(n,opt_precision){var i=n|0;var vf=getVF(n,opt_precision);if(i==1&&vf.v==0){return PLURAL_CATEGORY.ONE}return PLURAL_CATEGORY.OTHER}})}]);jqLite(window.document).ready(function(){angularInit(window.document,bootstrap)})})(window);!window.angular.$$csp().noInlineStyle&&window.angular.element(document.head).prepend('<style type="text/css">@charset "UTF-8";[ng\\:cloak],[ng-cloak],[data-ng-cloak],[x-ng-cloak],.ng-cloak,.x-ng-cloak,.ng-hide:not(.ng-hide-animate){display:none !important;}ng\\:form{display:block;}.ng-animate-shim{visibility:hidden;}.ng-anchor{position:absolute;}</style>');"use strict";var emulatorServicesCompilationDate="Fri Oct 28 09:42:21 EDT 2016";var gamingPlatform;(function(gamingPlatform){var log;(function(log_1){var ILogLevel=(function(){function ILogLevel(){}
ILogLevel.ALWAYS='ALWAYS';ILogLevel.LOG='LOG';ILogLevel.INFO='INFO';ILogLevel.DEBUG='DEBUG';ILogLevel.WARN='WARN';ILogLevel.ERROR='ERROR';return ILogLevel})();var alwaysLogs=[];var lastLogs=[];var startTime=getCurrentTime();function getCurrentTime(){return window.performance?window.performance.now():new Date().getTime()}
log_1.getCurrentTime=getCurrentTime;function getLogEntry(args,logLevel,consoleFunc){var millisecondsFromStart=getCurrentTime()-startTime;var secondsFromStart=Math.round(millisecondsFromStart)/1000;var consoleArgs=['',secondsFromStart,' seconds:'].concat(args);return{millisecondsFromStart:millisecondsFromStart,args:args,logLevel:logLevel}}
function storeLog(args,logLevel,consoleFunc){if(lastLogs.length>100){lastLogs.shift()}
lastLogs.push(getLogEntry(args,logLevel,consoleFunc))}
function getLogs(){return lastLogs.concat(alwaysLogs)}
log_1.getLogs=getLogs;function alwaysLog(){var args=[];for(var _i=0;_i<arguments.length;_i++){args[_i-0]=arguments[_i]}
alwaysLogs.push(getLogEntry(args,ILogLevel.ALWAYS,console.log))}
log_1.alwaysLog=alwaysLog;function info(){var args=[];for(var _i=0;_i<arguments.length;_i++){args[_i-0]=arguments[_i]}
storeLog(args,ILogLevel.INFO,console.log)}
log_1.info=info;function debug(){var args=[];for(var _i=0;_i<arguments.length;_i++){args[_i-0]=arguments[_i]}
storeLog(args,ILogLevel.DEBUG,console.debug)}
log_1.debug=debug;function warn(){var args=[];for(var _i=0;_i<arguments.length;_i++){args[_i-0]=arguments[_i]}
storeLog(args,ILogLevel.WARN,console.warn)}
log_1.warn=warn;function error(){var args=[];for(var _i=0;_i<arguments.length;_i++){args[_i-0]=arguments[_i]}
storeLog(args,ILogLevel.ERROR,console.error)}
log_1.error=error;function log(){var args=[];for(var _i=0;_i<arguments.length;_i++){args[_i-0]=arguments[_i]}
storeLog(args,ILogLevel.LOG,console.log)}
log_1.log=log})(log=gamingPlatform.log||(gamingPlatform.log={}))})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){var stateService;(function(stateService){var game;var currentState;var lastState;var currentVisibleTo;var lastVisibleTo;var lastMove;var turnIndexBeforeMove;var turnIndex=0;var endMatchScores=null;var setTurnOrEndMatchCount=0;var playersInfo;var playMode="passAndPlay";var randomSeed;var moveNumber;var simulateServerDelayMilliseconds=10;function setSimulateServerDelayMilliseconds(_simulateServerDelayMilliseconds){simulateServerDelayMilliseconds=_simulateServerDelayMilliseconds}
stateService.setSimulateServerDelayMilliseconds=setSimulateServerDelayMilliseconds;function setPlayMode(_playMode){playMode=_playMode}
stateService.setPlayMode=setPlayMode;function setRandomSeed(_randomSeed){randomSeed=_randomSeed}
stateService.setRandomSeed=setRandomSeed;function setPlayers(_playersInfo){playersInfo=_playersInfo}
stateService.setPlayers=setPlayers;function initNewMatch(){if(!game){throwError("You must call setGame before any other method.")}
currentState={};lastState=null;currentVisibleTo={};lastVisibleTo=null;lastMove=[];turnIndexBeforeMove=0;turnIndex=0;endMatchScores=null;moveNumber=0}
stateService.initNewMatch=initNewMatch;function getKeys(object){if(Object&&Object.keys){return Object.keys(object)}
var keys=[];for(var key in object){keys.push(key)}
return keys}
function clone(obj){return angular.copy(obj)}
function isNull(obj){return obj===undefined||obj===null}
function throwError(){var args=[];for(var _i=0;_i<arguments.length;_i++){args[_i-0]=arguments[_i]}
gamingPlatform.log.error("Throwing an error with these arguments=",args);var msg=args.join(", ");throw new Error(msg)}
function getMoveForPlayerIndex(playerIndex,move){var moveForPlayer=[];for(var _i=0;_i<move.length;_i++){var operation=move[_i];if(!isNull(operation.set)&&!isNull(operation.set.visibleToPlayerIndexes)&&operation.set.visibleToPlayerIndexes.indexOf(playerIndex)===-1){moveForPlayer.push({set:{key:operation.set.key,value:null,visibleToPlayerIndexes:operation.set.visibleToPlayerIndexes}})}
else{moveForPlayer.push(operation)}}
return moveForPlayer}
function getStateForPlayerIndex(playerIndex,gameState,visibleTo){if(gameState===null){return null}
var result={};var keys=getKeys(gameState);for(var _i=0;_i<keys.length;_i++){var key=keys[_i];var visibleToPlayerIndexes=visibleTo[key];var value=null;if(isNull(visibleToPlayerIndexes)||visibleToPlayerIndexes.indexOf(playerIndex)>-1){value=gameState[key]}
result[key]=value}
return result}
function shuffle(keys){var keysCopy=keys.slice(0);var result=[];while(keysCopy.length>=1){var index=randomFromTo(0,keysCopy.length);var removed=keysCopy.splice(index,1)[0];result.push(removed)}
return result}
function randomFromTo(from,to){if(isNull(from)||isNull(to)||from>=to){throw new Error("In randomFromTo(from,to), you must have from<to, but from="+from+" to="+to)}
return Math.floor(Math.random()*(to-from)+from)}
stateService.randomFromTo=randomFromTo;function processApiOperation(operation){var key;var visibleToPlayerIndexes;if(!isNull(operation.set)){var opSet=operation.set;key=opSet.key;visibleToPlayerIndexes=opSet.visibleToPlayerIndexes;var value=opSet.value;if(isNull(key)||isNull(value)){throwError("Fields key and value in Set operation must be non null. operation="+angular.toJson(operation,!0))}
currentState[key]=value;if(visibleToPlayerIndexes){currentVisibleTo[key]=visibleToPlayerIndexes}
else{delete currentVisibleTo[key]}}
else if(!isNull(operation.setTurn)){var setTurn=operation.setTurn;turnIndex=setTurn.turnIndex;setTurnOrEndMatchCount++}
else if(!isNull(operation.setRandomInteger)){var setRandomInteger=operation.setRandomInteger;key=setRandomInteger.key;var from=setRandomInteger.from;var to=setRandomInteger.to;if(isNull(key)||isNull(from)||isNull(to)){throwError("Fields key, from, and to, in SetRandomInteger operation must be non null. operation="+angular.toJson(operation,!0))}
var randomValue=randomFromTo(from,to);currentState[key]=randomValue;delete currentVisibleTo[key]}
else if(!isNull(operation.setVisibility)){var setVisibility=operation.setVisibility;key=setVisibility.key;visibleToPlayerIndexes=setVisibility.visibleToPlayerIndexes;if(isNull(key)){throwError("Fields key in SetVisibility operation must be non null. operation="+angular.toJson(operation,!0))}
if(visibleToPlayerIndexes){currentVisibleTo[key]=visibleToPlayerIndexes}
else{delete currentVisibleTo[key]}}
else if(!isNull(operation['delete'])){var opDelete=operation['delete'];key=opDelete.key;if(isNull(key)){throwError("Field key in Delete operation must be non null. operation="+angular.toJson(operation,!0))}
delete currentState[key];delete currentVisibleTo[key]}
else if(!isNull(operation.shuffle)){var opShuffle=operation.shuffle;var keys=opShuffle.keys;if(isNull(keys)||keys.length===0){throwError("Field keys in Shuffle operation must be a non empty array. operation="+angular.toJson(operation,!0))}
var shuffledKeys=shuffle(keys);var oldGameState=clone(currentState);var oldVisibleTo=clone(currentVisibleTo);for(var j=0;j<shuffledKeys.length;j++){var fromKey=keys[j];var toKey=shuffledKeys[j];currentState[toKey]=oldGameState[fromKey];currentVisibleTo[toKey]=oldVisibleTo[fromKey]}}
else if(!isNull(operation.endMatch)){var endMatch=operation.endMatch;setTurnOrEndMatchCount++;var scores=endMatch.endMatchScores;if(isNull(scores)||scores.length!==playersInfo.length){throwError("Field scores in EndMatch operation must be an array of the same length as the number of players. operation="+angular.toJson(operation,!0))}
endMatchScores=scores;if(playMode==="onlyAIs"){gamingPlatform.$timeout(function(){initNewMatch()},1000)}}
else{throwError("Illegal operation, it must contain either set, setRandomInteger, setVisibility, delete, shuffle, or endMatch: "+angular.toJson(operation,!0))}}
function getYourPlayerIndex(){return playMode==="playWhite"?0:playMode==="playBlack"?1:playMode==="playViewer"?-2:playMode==="playAgainstTheComputer"||playMode==="onlyAIs"||playMode==="passAndPlay"?turnIndex:Number(playMode)}
function getMatchState(){return{turnIndexBeforeMove:turnIndexBeforeMove,turnIndex:turnIndex,endMatchScores:endMatchScores,moveNumber:moveNumber,randomSeed:randomSeed,lastMove:lastMove,lastState:lastState,currentState:currentState,lastVisibleTo:lastVisibleTo,currentVisibleTo:currentVisibleTo}}
stateService.getMatchState=getMatchState;function getNullIfUndefined(obj){return obj===undefined?null:obj}
function setMatchState(data){turnIndexBeforeMove=getNullIfUndefined(data.turnIndexBeforeMove);turnIndex=getNullIfUndefined(data.turnIndex);endMatchScores=getNullIfUndefined(data.endMatchScores);moveNumber=data.moveNumber?data.moveNumber:0;randomSeed=getNullIfUndefined(data.randomSeed);lastMove=getNullIfUndefined(data.lastMove);lastState=getNullIfUndefined(data.lastState);currentState=getNullIfUndefined(data.currentState);lastVisibleTo=getNullIfUndefined(data.lastVisibleTo);currentVisibleTo=getNullIfUndefined(data.currentVisibleTo)}
stateService.setMatchState=setMatchState;var lastSentUpdateUI=null;function delayedSendUpdateUi(){var yourPlayerIndex=getYourPlayerIndex();var moveForIndex=getMoveForPlayerIndex(yourPlayerIndex,lastMove);var stateBeforeMove=getStateForPlayerIndex(yourPlayerIndex,lastState,lastVisibleTo);var stateAfterMove=getStateForPlayerIndex(yourPlayerIndex,currentState,currentVisibleTo);var nextUpdateUI={move:moveForIndex,turnIndexBeforeMove:turnIndexBeforeMove,turnIndexAfterMove:turnIndex,stateBeforeMove:stateBeforeMove,stateAfterMove:stateAfterMove,numberOfPlayers:playersInfo.length,playersInfo:playersInfo,yourPlayerIndex:yourPlayerIndex,playMode:playMode,moveNumber:moveNumber,randomSeed:randomSeed,endMatchScores:endMatchScores};if(angular.equals(lastSentUpdateUI,nextUpdateUI))
return;lastSentUpdateUI=nextUpdateUI;if(lastMove.length>0&&game.isMoveOk({move:moveForIndex,turnIndexBeforeMove:turnIndexBeforeMove,turnIndexAfterMove:turnIndex,stateBeforeMove:stateBeforeMove,stateAfterMove:stateAfterMove,numberOfPlayers:playersInfo.length})!==!0){throwError("You declared a hacker for a legal move! move="+moveForIndex)}
game.updateUI(nextUpdateUI)}
function sendUpdateUi(){if(simulateServerDelayMilliseconds===0){delayedSendUpdateUi()}
else{gamingPlatform.$timeout(function(){delayedSendUpdateUi()},simulateServerDelayMilliseconds)}}
stateService.sendUpdateUi=sendUpdateUi;function makeMove(operations){if(!game){throwError("You must call setGame before any other method.")}
if(!operations){throwError("operations must be an array")}
if(turnIndex===-1){throwError("You cannot send a move after the game ended!")}
if(getYourPlayerIndex()!==turnIndex){throwError("Expected a move from turnIndex="+turnIndex+" but got the move from index="+getYourPlayerIndex())}
lastState=clone(currentState);lastVisibleTo=clone(currentVisibleTo);turnIndexBeforeMove=turnIndex;turnIndex=-1;lastMove=operations;moveNumber++;if(randomSeed&&Math.seedrandom){Math.seedrandom(randomSeed+moveNumber)}
setTurnOrEndMatchCount=0;for(var _i=0;_i<operations.length;_i++){var operation=operations[_i];processApiOperation(operation)}
if(setTurnOrEndMatchCount!==1){throwError("We must have either SetTurn or EndMatch, but not both: setTurnOrEndMatchCount="+setTurnOrEndMatchCount)}
if(moveNumber>1&&turnIndex==turnIndexBeforeMove){}
if(!(turnIndex>=-1&&turnIndex<playersInfo.length)){}
sendUpdateUi()}
stateService.makeMove=makeMove;function setGame(_game){if(game!==undefined){throwError("You can call setGame only once")}
game=_game}
stateService.setGame=setGame;function getEndMatchScores(){return endMatchScores}
stateService.getEndMatchScores=getEndMatchScores})(stateService=gamingPlatform.stateService||(gamingPlatform.stateService={}))})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){var messageService;(function(messageService){var gameUrl=location.toString();function sendMessage(message){gamingPlatform.log.info("Game sent message",message);message.gameUrl=gameUrl;window.parent.postMessage(message,"*")}
messageService.sendMessage=sendMessage;function addMessageListener(listener){window.addEventListener("message",function(event){var source=event.source;if(source!==window.parent){return}
var message=event.data;gamingPlatform.log.info("Game got message",message);gamingPlatform.$rootScope.$apply(function(){listener(message)})},!1)}
messageService.addMessageListener=addMessageListener})(messageService=gamingPlatform.messageService||(gamingPlatform.messageService={}))})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){var gameService;(function(gameService){var isLocalTesting=window.parent===window||window.location.search==="?test";gameService.playMode=location.search.indexOf("onlyAIs")!==-1?"onlyAIs":location.search.indexOf("playAgainstTheComputer")!==-1?"playAgainstTheComputer":location.search.indexOf("?playMode=")===0?location.search.substr("?playMode=".length):"passAndPlay";var lastUpdateUI=null;var game;function updateUI(params){lastUpdateUI=angular.copy(params);game.updateUI(params)}
gameService.updateUI=updateUI;function makeMove(move){if(!lastUpdateUI){throw new Error("Game called makeMove before getting updateUI or it called makeMove more than once for a single updateUI.")}
var wasYourTurn=lastUpdateUI.turnIndexAfterMove>=0&&lastUpdateUI.yourPlayerIndex===lastUpdateUI.turnIndexAfterMove;if(!wasYourTurn){throw new Error("Game called makeMove when it wasn't your turn: yourPlayerIndex="+lastUpdateUI.yourPlayerIndex+" turnIndexAfterMove="+lastUpdateUI.turnIndexAfterMove)}
if(!move||!move.length){throw new Error("Game called makeMove with an empty move="+move)}
if(isLocalTesting){gamingPlatform.$timeout(function(){gamingPlatform.stateService.makeMove(move)},10)}
else{gamingPlatform.messageService.sendMessage({makeMove:move,lastUpdateUI:lastUpdateUI})}
lastUpdateUI=null}
gameService.makeMove=makeMove;function getPlayers(){var playersInfo=[];var actualNumberOfPlayers=gamingPlatform.stateService.randomFromTo(game.minNumberOfPlayers,game.maxNumberOfPlayers+1);for(var i=0;i<actualNumberOfPlayers;i++){var playerId=gameService.playMode==="onlyAIs"||i!==0&&gameService.playMode==="playAgainstTheComputer"?"":""+(i+42);playersInfo.push({playerId:playerId,avatarImageUrl:null,displayName:null})}
return playersInfo}
var didCallSetGame=!1;var w=window;function setGame(_game){game=_game;if(didCallSetGame){throw new Error("You can call setGame exactly once!")}
didCallSetGame=!0;var playersInfo=getPlayers();if(isLocalTesting){if(w.game){w.game.isHelpModalShown=!0}
gamingPlatform.stateService.setGame({updateUI:updateUI,isMoveOk:game.isMoveOk});gamingPlatform.stateService.initNewMatch();gamingPlatform.stateService.setPlayMode(gameService.playMode);gamingPlatform.stateService.setPlayers(playersInfo);gamingPlatform.stateService.sendUpdateUi()}
else{gamingPlatform.messageService.addMessageListener(function(message){if(message.isMoveOk){var isMoveOkResult=game.isMoveOk(message.isMoveOk);if(isMoveOkResult!==!0){isMoveOkResult={result:isMoveOkResult,isMoveOk:message.isMoveOk}}
gamingPlatform.messageService.sendMessage({isMoveOkResult:isMoveOkResult})}
else if(message.updateUI){updateUI(message.updateUI)}
else if(message.setLanguage){gamingPlatform.translate.setLanguage(message.setLanguage.language,message.setLanguage.codeToL10N);setTimeout(function(){gamingPlatform.messageService.sendMessage({setLanguageResult:!0})})}
else if(message.getGameLogs){var plainPojoLogs=angular.fromJson(angular.toJson(gamingPlatform.log.getLogs()));setTimeout(function(){gamingPlatform.messageService.sendMessage({getGameLogsResult:plainPojoLogs})})}
else if(message.getStateForOgImage){gamingPlatform.messageService.sendMessage({sendStateForOgImage:game.getStateForOgImage()})}
else if(message.passMessageToGame){var msgFromPlatform=message.passMessageToGame;if(msgFromPlatform.SHOW_GAME_INSTRUCTIONS&&w.game){w.game.isHelpModalShown=!w.game.isHelpModalShown}
if(game.gotMessageFromPlatform)
game.gotMessageFromPlatform(msgFromPlatform)}});gamingPlatform.messageService.sendMessage({gameReady:{}})}
gamingPlatform.log.info("Passing a 'fake' updateUI message in order to show an empty board to a viewer (so you can NOT perform moves)");updateUI({move:[],turnIndexBeforeMove:0,turnIndexAfterMove:0,stateBeforeMove:null,stateAfterMove:{},yourPlayerIndex:-2,playersInfo:playersInfo,playMode:"passAndPlay",endMatchScores:null,moveNumber:0,randomSeed:"",numberOfPlayers:playersInfo.length})}
gameService.setGame=setGame})(gameService=gamingPlatform.gameService||(gamingPlatform.gameService={}))})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){var moveService;(function(moveService){var STATE_KEY="state";function convertOldState(state){return state?state[STATE_KEY]:null}
function convertIsMoveOk(params){return{turnIndexBeforeMove:params.turnIndexBeforeMove,numberOfPlayers:params.numberOfPlayers,stateBeforeMove:convertOldState(params.stateBeforeMove),move:convertOldMove(params.move)}}
function convertUpdate(params){return{playersInfo:params.playersInfo,yourPlayerIndex:params.yourPlayerIndex,playMode:params.playMode,turnIndexBeforeMove:params.turnIndexBeforeMove,numberOfPlayers:params.numberOfPlayers,stateBeforeMove:convertOldState(params.stateBeforeMove),move:{endMatchScores:params.endMatchScores,turnIndexAfterMove:params.turnIndexAfterMove,stateAfterMove:convertOldState(params.stateAfterMove),},}}
function convertOldMove(move){if(!move||move.length===0){return{endMatchScores:null,turnIndexAfterMove:0,stateAfterMove:null,}}
if(move.length!==2||!(move[0].setTurn||move[0].endMatch)||!move[1].set){throw new Error("Internal error: old move should be an array with 2 operations! old move="+angular.toJson(move,!0))}
return{endMatchScores:move[0].endMatch?move[0].endMatch.endMatchScores:null,turnIndexAfterMove:move[0].setTurn?move[0].setTurn.turnIndex:-1,stateAfterMove:move[1].set.value,}}
function convertNewMove(move){var noTurnIndexAfterMove=move.turnIndexAfterMove===-1;var hasEndMatchScores=!!move.endMatchScores;if(noTurnIndexAfterMove&&!hasEndMatchScores){throw new Error("Illegal move: turnIndexAfterMove was -1 but you forgot to set endMatchScores. Move="+angular.toJson(move,!0))}
if(hasEndMatchScores&&!noTurnIndexAfterMove){throw new Error("Illegal move: you set endMatchScores but you didn't set turnIndexAfterMove to -1. Move="+angular.toJson(move,!0))}
return[hasEndMatchScores?{endMatch:{endMatchScores:move.endMatchScores}}:{setTurn:{turnIndex:move.turnIndexAfterMove}},{set:{key:STATE_KEY,value:move.stateAfterMove}}]}
function setGame(game){var oldGame={minNumberOfPlayers:game.minNumberOfPlayers,maxNumberOfPlayers:game.maxNumberOfPlayers,isMoveOk:function(params){var move=convertIsMoveOk(params);gamingPlatform.log.info("Calling game.checkMoveOk:",move);return!0},updateUI:function(params){var newParams=convertUpdate(params);gamingPlatform.log.info("Calling game.updateUI:",newParams);game.updateUI(newParams)},gotMessageFromPlatform:game.gotMessageFromPlatform,getStateForOgImage:game.getStateForOgImage,};gamingPlatform.gameService.setGame(oldGame)}
moveService.setGame=setGame;function makeMove(move){gamingPlatform.log.info("Making move:",move);gamingPlatform.gameService.makeMove(convertNewMove(move))}
moveService.makeMove=makeMove})(moveService=gamingPlatform.moveService||(gamingPlatform.moveService={}))})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){function resizeMapArea(params){var imageId=params.imageId;var mapId=params.mapId;var originalWidth=params.originalWidth;var originalHeight=params.originalHeight;function rescale(){var image=document.getElementById(imageId);var map=document.getElementById(mapId);var widthScale=image.width/originalWidth;var heightScale=image.height/originalHeight;var areaElements=map.getElementsByTagName("area");for(var areaIndex=0;areaIndex<areaElements.length;areaIndex++){var areaElement=areaElements[areaIndex];var originalCoords=areaElement.getAttribute("data-original-coords");if(!originalCoords){areaElement.setAttribute("data-original-coords",areaElement.getAttribute("coords"))}
var coords=areaElement.getAttribute("data-original-coords").split(',');var coordsPercent=[];for(var i=0;i<coords.length;++i){var coordNum=Number(coords[i]);if(i%2===0){coordsPercent[i]=Math.round(coordNum*widthScale)}
else{coordsPercent[i]=Math.round(coordNum*heightScale)}}
areaElement.setAttribute("coords",coordsPercent.toString())}}
document.addEventListener("onresize",rescale);document.addEventListener("orientationchange",rescale);setInterval(rescale,1000)}
gamingPlatform.resizeMapArea=resizeMapArea})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){var alphaBetaService;(function(alphaBetaService){function alphaBetaDecision(startingState,playerIndex,getNextStates,getStateScoreForIndex0,getDebugStateToString,alphaBetaLimits){var move=alphaBetaDecisionMayReturnNull(startingState,playerIndex,getNextStates,getStateScoreForIndex0,getDebugStateToString,alphaBetaLimits);if(move){return move}
return getNextStates(startingState,playerIndex)[0]}
alphaBetaService.alphaBetaDecision=alphaBetaDecision;function alphaBetaDecisionMayReturnNull(startingState,playerIndex,getNextStates,getStateScoreForIndex0,getDebugStateToString,alphaBetaLimits){if(!startingState||!getNextStates||!getStateScoreForIndex0){throw new Error("startingState or getNextStates or getStateScoreForIndex0 is null/undefined")}
if(playerIndex!==0&&playerIndex!==1){throw new Error("playerIndex must be either 0 or 1")}
if(!alphaBetaLimits.millisecondsLimit&&!alphaBetaLimits.maxDepth){throw new Error("alphaBetaLimits must have either millisecondsLimit or maxDepth")}
if(alphaBetaLimits.millisecondsLimit){alphaBetaLimits.millisecondsLimit=Math.min(400,alphaBetaLimits.millisecondsLimit)}
var startTime=new Date().getTime();if(alphaBetaLimits.maxDepth){return getScoreForIndex0(startingState,playerIndex,getNextStates,getStateScoreForIndex0,getDebugStateToString,alphaBetaLimits,startTime,0,Number.NEGATIVE_INFINITY,Number.POSITIVE_INFINITY).bestState}
if(getDebugStateToString!=null){console.log("Doing iterative-deepeninh (A*) until we run out of time or find a certain win/lose move.")}
var maxDepth=1;var bestState;while(!0){if(getDebugStateToString!=null){console.log("Alpha-beta search until maxDepth="+maxDepth)}
var nextBestStateAndScore=getScoreForIndex0(startingState,playerIndex,getNextStates,getStateScoreForIndex0,getDebugStateToString,{maxDepth:maxDepth,millisecondsLimit:alphaBetaLimits.millisecondsLimit},startTime,0,Number.NEGATIVE_INFINITY,Number.POSITIVE_INFINITY);var nextBestScore=nextBestStateAndScore.bestScore;var nextBestState=nextBestStateAndScore.bestState;if(nextBestScore===Number.POSITIVE_INFINITY||nextBestScore===Number.NEGATIVE_INFINITY){var isWin=nextBestScore===(playerIndex===0?Number.POSITIVE_INFINITY:Number.NEGATIVE_INFINITY);console.log("Discovered that AI is going to "+(isWin?"win":"lose")+" with maxDepth="+maxDepth);if(getDebugStateToString!=null){console.log("Best state is "+getDebugStateToString(nextBestState))}
return nextBestState}
var isHalfTimePassed=isTimeout({millisecondsLimit:alphaBetaLimits.millisecondsLimit/2},startTime);var isAllTimePassed=isTimeout(alphaBetaLimits,startTime);if(isHalfTimePassed||isAllTimePassed){var result=!isAllTimePassed||maxDepth===1?nextBestState:bestState;if(isAllTimePassed){console.log("Run out of time when maxDepth="+maxDepth+", so returning the best state for maxDepth="+(maxDepth===1?1:maxDepth-1))}
else{console.log("Run out of half the time when maxDepth="+maxDepth+", so no point of exploring the next depth.")}
if(getDebugStateToString!=null){console.log("Best state is "+getDebugStateToString(result))}
return result}
bestState=nextBestState;maxDepth++}}
function isTimeout(alphaBetaLimits,startTime){return alphaBetaLimits.millisecondsLimit&&new Date().getTime()-startTime>alphaBetaLimits.millisecondsLimit}
function getScoreForIndex0(startingState,playerIndex,getNextStates,getStateScoreForIndex0,getDebugStateToString,alphaBetaLimits,startTime,depth,alpha,beta){var bestScore=null;var bestState=null;if(isTimeout(alphaBetaLimits,startTime)){if(getDebugStateToString!=null){console.log("Run out of time, just quitting from this traversal.")}
return{bestScore:0,bestState:null}}
if(depth===alphaBetaLimits.maxDepth){bestScore=getStateScoreForIndex0(startingState,playerIndex);if(getDebugStateToString!=null){console.log("Max depth reached, score is "+bestScore)}
return{bestScore:bestScore,bestState:null}}
var states=getNextStates(startingState,playerIndex);if(getDebugStateToString!=null){console.log(getDebugStateToString(startingState)+" has "+states.length+" next states")}
if(states.length===0){bestScore=getStateScoreForIndex0(startingState,playerIndex);if(getDebugStateToString!=null){console.log("Terminal state, score is "+bestScore)}
return{bestScore:bestScore,bestState:null}}
for(var i=0;i<states.length;i++){var state=states[i];var scoreForIndex0=getScoreForIndex0(state,1-playerIndex,getNextStates,getStateScoreForIndex0,getDebugStateToString,alphaBetaLimits,startTime,depth+1,alpha,beta).bestScore;if(getDebugStateToString!=null){console.log("Score of "+getDebugStateToString(state)+" is "+scoreForIndex0)}
if(bestScore===null||playerIndex===0&&scoreForIndex0>bestScore||playerIndex===1&&scoreForIndex0<bestScore){bestScore=scoreForIndex0;bestState=state}
if(playerIndex===0){if(bestScore>=beta){return{bestScore:bestScore,bestState:bestState}}
alpha=Math.max(alpha,bestScore)}
else{if(bestScore<=alpha){return{bestScore:bestScore,bestState:bestState}}
beta=Math.min(beta,bestScore)}}
if(getDebugStateToString!=null){console.log("Best next state for playerIndex "+playerIndex+" is "+getDebugStateToString(bestState)+" with score of "+bestScore)}
return{bestScore:bestScore,bestState:bestState}}})(alphaBetaService=gamingPlatform.alphaBetaService||(gamingPlatform.alphaBetaService={}))})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){var resizeGameAreaService;(function(resizeGameAreaService){var widthToHeight=null;var dimensionsChanged=null;var oldSizes=null;var doc=window.document;var gameArea;function setWidthToHeight(_widthToHeight,_dimensionsChanged){gamingPlatform.log.info("setWidthToHeight to ",_widthToHeight);widthToHeight=_widthToHeight;dimensionsChanged=_dimensionsChanged;gameArea=doc.getElementById('gameArea');if(!gameArea){throw new Error("You forgot to add to your <body> this div: <div id='gameArea'>...</div>")}
oldSizes=null;rescale();setTimeout(rescale,10);setTimeout(rescale,100)}
resizeGameAreaService.setWidthToHeight=setWidthToHeight;function round2(num){return Math.round(num*100)/100}
function rescale(){if(widthToHeight===null){return}
var originalWindowWidth=window.innerWidth;var originalWindowHeight=window.innerHeight;var windowWidth=originalWindowWidth;var windowHeight=originalWindowHeight;if(oldSizes!==null){if(oldSizes.windowWidth===windowWidth&&oldSizes.windowHeight===windowHeight){return}}
oldSizes={windowWidth:windowWidth,windowHeight:windowHeight};if(windowWidth===0||windowHeight===0){gamingPlatform.log.info("Window width/height is 0 so hiding gameArea div.");gameArea.style.display="none";return}
gameArea.style.display="block";gamingPlatform.$rootScope.$apply(function(){var newWidthToHeight=windowWidth/windowHeight;if(newWidthToHeight>widthToHeight){windowWidth=round2(windowHeight*widthToHeight)}
else{windowHeight=round2(windowWidth/widthToHeight)}
gamingPlatform.log.info("Window size is "+oldSizes.windowWidth+"x"+oldSizes.windowHeight+" so setting gameArea size to "+windowWidth+"x"+windowHeight+" because widthToHeight="+widthToHeight);var keepMargin=0.95;windowWidth*=keepMargin;windowHeight*=keepMargin;gameArea.style.width=windowWidth+'px';gameArea.style.height=windowHeight+'px';gameArea.style.position="absolute";gameArea.style.left=((originalWindowWidth-windowWidth)/2)+'px';gameArea.style.top=((originalWindowHeight-windowHeight)/2)+'px';if(dimensionsChanged)
dimensionsChanged(windowWidth,windowHeight);setTimeout(rescale,10)})}
doc.addEventListener("onresize",rescale);doc.addEventListener("orientationchange",rescale);if(window.matchMedia)
window.matchMedia('(orientation: portrait)').addListener(rescale);setInterval(rescale,300)})(resizeGameAreaService=gamingPlatform.resizeGameAreaService||(gamingPlatform.resizeGameAreaService={}))})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){function createTranslateService(){var language;var codeToL10N=null;var idToLanguageToL10n=null;function translate(translationId,interpolateParams,languageCode){if(!languageCode)
languageCode=language;var translation=null;if(idToLanguageToL10n&&idToLanguageToL10n[translationId]){var languageToL10n=idToLanguageToL10n[translationId];translation=languageToL10n[languageCode];if(!translation)
translation=languageToL10n.en}
else if(codeToL10N){translation=codeToL10N[translationId]}
if(!translation){translation="["+translationId+"]";gamingPlatform.log.error("Couldn't find translationId="+translationId+" in language="+languageCode)}
var result=gamingPlatform.$interpolate(translation)(interpolateParams||{});if(result.indexOf('{{')!==-1){gamingPlatform.log.error("You forgot to pass a translation parameter (interpolateParams) for translationId="+translationId+" in language="+languageCode+" which resulted in '"+result+"' (note that you forgot to pass some {{XXX}})")}
return result}
var translateService;translateService=translate;translateService.getLanguage=function(){return language};translateService.setTranslations=function(_idToLanguageToL10n){idToLanguageToL10n=_idToLanguageToL10n};translateService.setLanguage=function(_language,_codeToL10N){language=_language;codeToL10N=_codeToL10N};return translateService}
gamingPlatform.translate=createTranslateService();gamingPlatform.defaultTranslateInterpolateParams={};angular.module('translate',[]).filter('translate',['$parse',function($parse){var translateFilter=function(translationId,interpolateParams){if(!angular.isObject(interpolateParams)){interpolateParams=$parse(interpolateParams)(this)}
if(!interpolateParams)
interpolateParams=gamingPlatform.defaultTranslateInterpolateParams;return gamingPlatform.translate(translationId,interpolateParams)};translateFilter.$stateful=!0;return translateFilter}])})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){var dragAndDropService;(function(dragAndDropService){function addDragListener(touchElementId,handleDragEvent){if(!touchElementId||!handleDragEvent){throw new Error("When calling addDragListener(touchElementId, handleDragEvent), you must pass two parameters")}
var isMouseDown=!1;function touchHandler(event){var touch=event.changedTouches[0];handleEvent(event,event.type,touch.clientX,touch.clientY)}
function mouseDownHandler(event){isMouseDown=!0;handleEvent(event,"touchstart",event.clientX,event.clientY)}
function mouseMoveHandler(event){if(isMouseDown){handleEvent(event,"touchmove",event.clientX,event.clientY)}}
function mouseUpHandler(event){isMouseDown=!1;handleEvent(event,"touchend",event.clientX,event.clientY)}
function handleEvent(event,type,clientX,clientY){if(event.preventDefault){event.preventDefault()}
if(event.stopPropagation){event.stopPropagation()}
event.cancelBubble=!0;event.returnValue=!1;console.log("handleDragEvent:",type,clientX,clientY);handleDragEvent(type,clientX,clientY,event)}
var gameArea=document.getElementById(touchElementId);if(!gameArea){throw new Error("You must have <div id='"+touchElementId+"'>...</div>")}
gameArea.addEventListener("touchstart",touchHandler,!0);gameArea.addEventListener("touchmove",touchHandler,!0);gameArea.addEventListener("touchend",touchHandler,!0);gameArea.addEventListener("touchcancel",touchHandler,!0);gameArea.addEventListener("touchleave",touchHandler,!0);gameArea.addEventListener("mousedown",mouseDownHandler,!0);gameArea.addEventListener("mousemove",mouseMoveHandler,!0);gameArea.addEventListener("mouseup",mouseUpHandler,!0)}
dragAndDropService.addDragListener=addDragListener})(dragAndDropService=gamingPlatform.dragAndDropService||(gamingPlatform.dragAndDropService={}))})(gamingPlatform||(gamingPlatform={}));var gamingPlatform;(function(gamingPlatform){function copyNamespaceToWindow(){var w=window;var g=gamingPlatform;for(var key in g){w[key]=g[key]}}
copyNamespaceToWindow();setTimeout(copyNamespaceToWindow,0);window.oncontextmenu=function(event){event.preventDefault();event.stopPropagation();return!1};angular.module('gameServices',['translate']).config(['$provide',function($provide){$provide.decorator('$rootScope',['$delegate',function($delegate){$delegate.unsafeOldApply=$delegate.$apply;$delegate.$apply=function(fn){var phase=$delegate.$$phase;if(phase==="$apply"||phase==="$digest"){if(fn&&typeof fn==='function'){fn()}}
else{$delegate.unsafeOldApply(fn)}};return $delegate}])}]).run(['$location','$rootScope','$timeout','$interval','$interpolate',function(_location,_rootScope,_timeout,_interval,_interpolate){gamingPlatform.$location=_location;gamingPlatform.$rootScope=_rootScope;gamingPlatform.$timeout=_timeout;gamingPlatform.$interval=_interval;gamingPlatform.$interpolate=_interpolate;copyNamespaceToWindow();gamingPlatform.log.alwaysLog("Finished init of gameServices module; emulatorServicesCompilationDate=",emulatorServicesCompilationDate)}]).factory('$exceptionHandler',function(){function angularErrorHandler(exception,cause){var errMsg={gameUrl:''+window.location,exception:""+exception,stack:""+(exception?exception.stack:"no stack"),cause:cause,gameLogs:gamingPlatform.log.getLogs()};console.error("Game had an exception:\n",exception," Full error message with logs: ",errMsg);window.alert("Game had an unexpected error. If you know JavaScript, you can look at the console and try to debug it :)");var plainPojoErr=angular.fromJson(angular.toJson(errMsg));window.parent.postMessage({emailJavaScriptError:plainPojoErr},"*")}
window.onerror=function(errorMsg,url,lineNumber,column,errorObj){angularErrorHandler(errorObj,'Error: '+errorMsg+' Script: '+url+' Line: '+lineNumber+' Column: '+column)};return angularErrorHandler})})(gamingPlatform||(gamingPlatform={}));var gameLogic;(function(gameLogic){gameLogic.ROWS=30;gameLogic.COLS=30;gameLogic.NumberOfBarrier=15;gameLogic.NumberOfPlayer=2;gameLogic.NumberOfFood=15;function getInitialBoardAndSnakes(){var board=getInitialBoardWithBarriersAndFoods();var snakes=[];for(var i=0;i<gameLogic.NumberOfPlayer;i++){snakes[i]=getInitialSnake(board,i+1)}
return{board:board,snakes:snakes}}
function getInitialBoardWithBarriersAndFoods(){var board=[];for(var i=0;i<gameLogic.ROWS;i++){board[i]=[];for(var j=0;j<gameLogic.COLS;j++){board[i][j]=''}}
var curBarriers=0;var curFoods=0;while(curBarriers<gameLogic.NumberOfBarrier||curFoods<gameLogic.NumberOfFood){var randomX=Math.floor((Math.random()*gameLogic.ROWS));var randomY=Math.floor((Math.random()*gameLogic.COLS));if(board[randomX][randomY]==''){if(curBarriers<gameLogic.NumberOfBarrier){board[randomX][randomY]='BARRIER';curBarriers++}
else{board[randomX][randomY]='FOOD';curFoods++}}}
return board}
function getInitialSnake(board,player){var snake={headToTail:[],dead:!1,oldTail:null,currentDirection:null,loseInfo:''};var found=!1;while(!found){var randomX=Math.floor((Math.random()*gameLogic.ROWS));var randomY=Math.floor((Math.random()*gameLogic.COLS));if(board[randomX][randomY]===''){snake.headToTail=[{row:randomX,col:randomY}];found=!0;var count=0;var direction=null;if(!isBarrierOrBorderOrOpponentOrMySelf(randomX+1,randomY,board)){count++;if(Math.floor((Math.random()*count))==0){direction={shiftX:1,shiftY:0}}}
if(!isBarrierOrBorderOrOpponentOrMySelf(randomX-1,randomY,board)){count++;if(Math.floor((Math.random()*count))==0){direction={shiftX:-1,shiftY:0}}}
if(!isBarrierOrBorderOrOpponentOrMySelf(randomX,randomY+1,board)){count++;if(Math.floor((Math.random()*count))==0){direction={shiftX:0,shiftY:1}}}
if(!isBarrierOrBorderOrOpponentOrMySelf(randomX,randomY-1,board)){count++;if(Math.floor((Math.random()*count))==0){direction={shiftX:0,shiftY:-1}}}
snake.currentDirection=direction;if(count==0){found=!1;snake.headToTail=[]}}}
board[snake.headToTail[0].row][snake.headToTail[0].col]="SNAKE"+player;return snake}
function isBarrierOrBorderOrOpponentOrMySelf(x,y,board){if(x<0||x>=gameLogic.ROWS||y<0||y>=gameLogic.COLS){return!0}
if(board[x][y]===''||board[x][y]==='FOOD'){return!1}
return!0}
gameLogic.isBarrierOrBorderOrOpponentOrMySelf=isBarrierOrBorderOrOpponentOrMySelf;function getInitialState(){return{boardWithSnakes:getInitialBoardAndSnakes(),newDirections:[]}}
gameLogic.getInitialState=getInitialState;function isTie(boardWithSnakes,time){var allDead=!0;for(var _i=0,_a=boardWithSnakes.snakes;_i<_a.length;_i++){var snake=_a[_i];if(!snake.dead){allDead=!1}}
if(allDead){return!0}
if(time<=0){var isTie_1=!0;for(var i=0;i<boardWithSnakes.snakes.length-1;i++){if(boardWithSnakes.snakes[i].headToTail.length!=boardWithSnakes.snakes[i+1].headToTail.length){isTie_1=!1}}
return isTie_1}
return!1}
function getWinner(boardWithSnakes,time){if(time>0){var winner='';var countOfLiveSnake=0;for(var i=0;i<boardWithSnakes.snakes.length;i++){if(!boardWithSnakes.snakes[i].dead){countOfLiveSnake++;winner=i+1+''}}
if(countOfLiveSnake!=1){winner=''}
return winner}
else{var maxLen=0;var winner='';var draw=!1;for(var i=0;i<boardWithSnakes.snakes.length;i++){if(!boardWithSnakes.snakes[i].dead){if(boardWithSnakes.snakes[i].headToTail.length>maxLen){maxLen=boardWithSnakes.snakes[i].headToTail.length;winner=i+1+'';draw=!1}
else if(boardWithSnakes.snakes[i].headToTail.length==maxLen){draw=!0}}}
return draw?'':winner}}
gameLogic.getWinner=getWinner;function createMove(stateBeforeMove,newDirections,leftTime,turnIndexBeforeMove){var end=!1;if(!stateBeforeMove){stateBeforeMove=getInitialState()}
var boardWithSnakes=stateBeforeMove.boardWithSnakes;if(getWinner(boardWithSnakes,leftTime)!==''||isTie(boardWithSnakes,leftTime)){throw new Error("Can only make a move if the game is not over!")}
var boardWithSnakesAfterMove={board:angular.copy(boardWithSnakes.board),snakes:angular.copy(boardWithSnakes.snakes)};if(leftTime<=0){end=!0}
else{for(var index=0;index<newDirections.length;index++){if(boardWithSnakesAfterMove.snakes[index].dead){continue}
var head=boardWithSnakesAfterMove.snakes[index].headToTail[0];var newHeadX=void 0;var newHeadY=void 0;var oldDirection=boardWithSnakesAfterMove.snakes[index].currentDirection;if(newDirections[index]!=null)
log.info("ddd",oldDirection.shiftX,oldDirection.shiftY,newDirections[index].shiftX,newDirections[index].shiftY);if(newDirections[index]==null||(Math.abs(oldDirection.shiftX)==Math.abs(newDirections[index].shiftX)&&Math.abs(oldDirection.shiftY)==Math.abs(newDirections[index].shiftY))){newHeadX=head.row+boardWithSnakesAfterMove.snakes[index].currentDirection.shiftX;newHeadY=head.col+boardWithSnakesAfterMove.snakes[index].currentDirection.shiftY}
else{newHeadX=head.row+newDirections[index].shiftX;newHeadY=head.col+newDirections[index].shiftY;boardWithSnakesAfterMove.snakes[index].currentDirection=newDirections[index]}
boardWithSnakesAfterMove.snakes[index].headToTail.unshift({row:newHeadX,col:newHeadY});if((newHeadX<0||newHeadX>=gameLogic.ROWS||newHeadY<0||newHeadY>=gameLogic.COLS)||boardWithSnakesAfterMove.board[newHeadX][newHeadY]!=='FOOD'){boardWithSnakesAfterMove.snakes[index].oldTail=boardWithSnakesAfterMove.snakes[index].headToTail.pop()}}
for(var index=0;index<boardWithSnakesAfterMove.snakes.length;index++){var board=boardWithSnakesAfterMove.board;var snake=boardWithSnakesAfterMove.snakes[index];var head=snake.headToTail[0];if(head.row<0||head.row>=gameLogic.ROWS||head.col<0||head.col>=gameLogic.COLS){snake.dead=!0;snake.loseInfo='bump into wall.';log.log("dead because bump into border");continue}
if(board[head.row][head.col]==='BARRIER'){snake.dead=!0;snake.loseInfo='bump into obstacle.';log.log("dead because bump into barrier");continue}
for(var i=1;i<snake.headToTail.length;i++){if(snake.headToTail[i].row==head.row&&snake.headToTail[i].col==head.col){snake.loseInfo='bump into itself';snake.dead=!0;break}}
outer:for(var secondIndex=0;secondIndex<boardWithSnakesAfterMove.snakes.length;secondIndex++){if(secondIndex!=index){var anotherSnake=boardWithSnakesAfterMove.snakes[secondIndex];for(var j=0;j<anotherSnake.headToTail.length;j++){if(anotherSnake.headToTail[j].row==head.row&&anotherSnake.headToTail[j].col==head.col){snake.loseInfo='bump into others';snake.dead=!0;break outer}}}}}
updateBoardWithSnakes(boardWithSnakesAfterMove)}
var winner=getWinner(boardWithSnakesAfterMove,leftTime);var matchScores=[];if(winner!==''||isTie(boardWithSnakesAfterMove,leftTime)){for(var _i=0,_a=boardWithSnakesAfterMove.snakes;_i<_a.length;_i++){var snake=_a[_i];matchScores.push(snake.headToTail.length)}
end=!0}
var stateAfterMove={newDirections:newDirections,boardWithSnakes:boardWithSnakesAfterMove};return{stateAfterMove:stateAfterMove,end:end,turnIndexAfterMove:gameLogic.NumberOfPlayer-1-turnIndexBeforeMove}}
gameLogic.createMove=createMove;function updateBoardWithSnakes(boardWithSnakes){var foodEaten=0;for(var i=0;i<boardWithSnakes.snakes.length;i++){var snake=boardWithSnakes.snakes[i];if(!snake.dead){if(snake.oldTail!=null){log.info("I'm in old tail",snake.oldTail);if(boardWithSnakes.board[snake.oldTail.row][snake.oldTail.col]==="SNAKE"+(i+1)){boardWithSnakes.board[snake.oldTail.row][snake.oldTail.col]=''}}
if(boardWithSnakes.board[snake.headToTail[0].row][snake.headToTail[0].col]==='FOOD'){foodEaten++}
boardWithSnakes.board[snake.headToTail[0].row][snake.headToTail[0].col]="SNAKE"+(i+1);log.info("I'm in new head",snake.headToTail[0].row,snake.headToTail[0].col,boardWithSnakes.board[snake.headToTail[0].row][snake.headToTail[0].col])}
else{for(var i_1=1;i_1<snake.headToTail.length;i_1++){var cell=snake.headToTail[i_1];if(cell.row>=0&&cell.row<gameLogic.ROWS&&cell.col>=0&&cell.col<gameLogic.COLS){if(boardWithSnakes.board[cell.row][cell.col]!=='BARRIER'){boardWithSnakes.board[cell.row][cell.col]='STONE'}}}
if(snake.oldTail!=null){boardWithSnakes.board[snake.oldTail.row][snake.oldTail.col]='STONE'}}}
fillBoardWithFood(boardWithSnakes,foodEaten)}
function fillBoardWithFood(boardWithSnakes,foodEaten){while(foodEaten>0){var randomX=Math.floor((Math.random()*gameLogic.ROWS));var randomY=Math.floor((Math.random()*gameLogic.COLS));if(boardWithSnakes.board[randomX][randomY]===''){boardWithSnakes.board[randomX][randomY]='FOOD';foodEaten--}}}
function createInitialMove(){return{stateAfterMove:getInitialState(),end:!1,turnIndexAfterMove:0}}
gameLogic.createInitialMove=createInitialMove;function checkMoveOk(stateTransition){}
gameLogic.checkMoveOk=checkMoveOk})(gameLogic||(gameLogic={}));var game;(function(game){game.ALLTIME=120*1000;game.GameSpeed=500;game.BoardSize=gameLogic.ROWS;game.ComputerOrHuman=[1,-1];game.NumberOfFood=gameLogic.NumberOfFood;game.NumberOfBarrier=gameLogic.NumberOfBarrier;game.ThirdComputerPlayer=!1;game.currentUpdateUI=null;game.didMakeMove=!1;game.state=null;game.action=null;game.snakeOneMove=null;game.snakeTwoMove=null;game.snakeThreeMove=null;game.RemainingTime=game.ALLTIME;game.reset=!0;game.loseInfo='';function init(){resizeGameAreaService.setWidthToHeight(1);moveService.setGame({minNumberOfPlayers:2,maxNumberOfPlayers:3,updateUI:updateUI,gotMessageFromPlatform:null,})}
game.init=init;function updateUI(params){log.info("Game got updateUI:",params);game.didMakeMove=!1;game.currentUpdateUI=params;game.state=params.move.stateAfterMove;if(isFirstMove()){game.state=gameLogic.getInitialState()}}
game.updateUI=updateUI;function makeMove(move){game.reset=!1;if(game.didMakeMove){return}
game.didMakeMove=!0;moveService.makeMove(move)}
function isFirstMove(){return!game.currentUpdateUI.move.stateAfterMove}
function move(){if(window.location.search==='?throwException'){throw new Error("Throwing the error because URL has '?throwException'")}
computerMove();var nextMove=null;try{var tmpMove=[angular.copy(game.snakeOneMove),angular.copy(game.snakeTwoMove)];if(game.ThirdComputerPlayer){tmpMove.push(angular.copy(game.snakeThreeMove))}
nextMove=gameLogic.createMove(game.state,tmpMove,game.RemainingTime-=game.GameSpeed,game.currentUpdateUI.move.turnIndexAfterMove);game.snakeOneMove=null;game.snakeTwoMove=null;game.snakeThreeMove=null}
catch(e){$interval.cancel(game.action);game.currentUpdateUI.end=!0;log.error(e);try{sendResult()}
catch(e){log.info("send fail")}
return}
makeMove(nextMove)}
game.move=move;function sendResult(){var snake=game.state.boardWithSnakes.snakes;for(var i=0;i<game.ComputerOrHuman.length;i++){if(game.ComputerOrHuman[i]==1){var score=snake[i].headToTail.length;var url="../../../dbman/saveScore.php?"+"gamename=MultiplayerSnake&playername=player"+i+"&score="+score;httpGetAsync(url,function(){})}}}
...
callback(xmlHttp.responseText)};xmlHttp.open("GET",theUrl,!0);xmlHttp.send(null)}
function computerMove(){var computerMoves=aiService.findComputerMove(game.ComputerOrHuman,game.state.boardWithSnakes);if(game.ComputerOrHuman[0]==-1){game.snakeOneMove=computerMoves[0]}
if(game.ComputerOrHuman[1]==-1){game.snakeTwoMove=computerMoves[1]}
if(game.ComputerOrHuman[2]==-1){game.snakeThreeMove=computerMoves[2]}}
function resetEverything(){$interval.cancel(game.action);game.action=null;game.RemainingTime=game.ALLTIME;game.reset=!0;game.currentUpdateUI.move.stateAfterMove=null;game.currentUpdateUI.end=!1;updateUI(game.currentUpdateUI)}
function isFood(row,col){return game.state.boardWithSnakes.board[row][col]==='FOOD'}
game.isFood=isFood;function isBarrier(row,col){return game.state.boardWithSnakes.board[row][col]==='BARRIER'}
game.isBarrier=isBarrier;function isSnakeOne(row,col){return game.state.boardWithSnakes.board[row][col]==='SNAKE1'}
game.isSnakeOne=isSnakeOne;function isSnakeTwo(row,col){return game.state.boardWithSnakes.board[row][col]==='SNAKE2'}
game.isSnakeTwo=isSnakeTwo;function isSnakeThree(row,col){return game.state.boardWithSnakes.board[row][col]==='SNAKE3'}
game.isSnakeThree=isSnakeThree;function isDeadSnake(row,col){return game.state.boardWithSnakes.board[row][col]==='STONE'}
game.isDeadSnake=isDeadSnake;function getNumber(){var res=[];for(var i=0;i<gameLogic.ROWS;i++){res.push(i)}
return res}
game.getNumber=getNumber;function getSnakeLength(index){if(isFirstMove()){return 1}
else{return game.currentUpdateUI.move.stateAfterMove.boardWithSnakes.snakes[index].headToTail.length}}
game.getSnakeLength=getSnakeLength;function changeFoodNumber(){gameLogic.NumberOfFood=game.NumberOfFood;resetEverything()}
game.changeFoodNumber=changeFoodNumber;function changeBarrierNumber(){gameLogic.NumberOfBarrier=game.NumberOfBarrier;resetEverything()}
game.changeBarrierNumber=changeBarrierNumber;function changePlayerNumber(){if(game.ThirdComputerPlayer){game.ComputerOrHuman.push(-1);gameLogic.NumberOfPlayer=3}
else{game.ComputerOrHuman.pop();gameLogic.NumberOfPlayer=2}
resetEverything()}
game.changePlayerNumber=changePlayerNumber;function changeGameSpeed(){if(game.action){$interval.cancel(game.action);game.action=$interval(move,game.GameSpeed)}}
game.changeGameSpeed=changeGameSpeed;function isDraw(){if(game.currentUpdateUI.end==!0){return gameLogic.getWinner(game.currentUpdateUI.move.stateAfterMove.boardWithSnakes,game.RemainingTime)===''}
return!1}
game.isDraw=isDraw;function isFinished(){return game.currentUpdateUI.end}
game.isFinished=isFinished;function getWinnerColor(){var winner=gameLogic.getWinner(game.currentUpdateUI.move.stateAfterMove.boardWithSnakes,game.RemainingTime);if(winner==='1'){return 'blue'}
else if(winner==='2'){return 'red'}
else{return 'orange'}}
game.getWinnerColor=getWinnerColor;function isSnakeDead(index){if(isFirstMove()){return!1}
else{return game.currentUpdateUI.move.stateAfterMove.boardWithSnakes.snakes[index].dead}}
game.isSnakeDead=isSnakeDead;function getSnakeLoseInfo(index){if(isFirstMove()){return ''}
else{return game.currentUpdateUI.move.stateAfterMove.boardWithSnakes.snakes[index].loseInfo}}
game.getSnakeLoseInfo=getSnakeLoseInfo;function shouldSlowlyAppear(row,col){if(isFirstMove()||!game.currentUpdateUI.stateBeforeMove||game.reset){return!0}
return!1}
game.shouldSlowlyAppear=shouldSlowlyAppear;function shouldSlowlyDisappear(row,col){return!1}
game.shouldSlowlyDisappear=shouldSlowlyDisappear;function keyDown(keyCode){if(keyCode==13){if(game.currentUpdateUI.end){resetEverything()}
else if(game.action==null){game.action=$interval(move,game.GameSpeed)}
else{$interval.cancel(game.action);game.action=null}}
if(keyCode==82){resetEverything();if(game.action){$interval.cancel(game.action);game.action=null}}
if(game.ComputerOrHuman[0]==1){if(keyCode==38){if(game.snakeOneMove==null){game.snakeOneMove={shiftX:-1,shiftY:0}}}
if(keyCode==40){if(game.snakeOneMove==null){game.snakeOneMove={shiftX:1,shiftY:0}}}
if(keyCode==37){if(game.snakeOneMove==null){game.snakeOneMove={shiftX:0,shiftY:-1}}}
if(keyCode==39){if(game.snakeOneMove==null){game.snakeOneMove={shiftX:0,shiftY:1}}}}
if(game.ComputerOrHuman[1]==1){if(keyCode==87){if(game.snakeTwoMove==null){game.snakeTwoMove={shiftX:-1,shiftY:0}}}
if(keyCode==83){if(game.snakeTwoMove==null){game.snakeTwoMove={shiftX:1,shiftY:0}}}
if(keyCode==65){if(game.snakeTwoMove==null){game.snakeTwoMove={shiftX:0,shiftY:-1}}}
if(keyCode==68){if(game.snakeTwoMove==null){game.snakeTwoMove={shiftX:0,shiftY:1}}}}
if(game.currentUpdateUI.stateBeforeMove){if(game.ComputerOrHuman[0]==1&&game.snakeOneMove!=null){var oldDirection=game.currentUpdateUI.stateBeforeMove.boardWithSnakes.snakes[0].currentDirection;if((oldDirection.shiftX)==(game.snakeOneMove.shiftX)&&(oldDirection.shiftY)==(game.snakeOneMove.shiftY)){game.snakeOneMove=null}}
if(game.ComputerOrHuman[1]==1&&game.snakeTwoMove!=null){var oldDirection=game.currentUpdateUI.stateBeforeMove.boardWithSnakes.snakes[1].currentDirection;if((oldDirection.shiftX)==(game.snakeTwoMove.shiftX)&&(oldDirection.shiftY)==(game.snakeTwoMove.shiftY)){game.snakeTwoMove=null}}}}
game.keyDown=keyDown})(game||(game={}));angular.module('myApp',['gameServices']).run(function(){$rootScope.game=game;game.init()});var aiService;(function(aiService){function findComputerMove(computerOrHuman,boardWithSnake){var current=new Date().getTime();var res=[];for(var i=0;i<computerOrHuman.length;i++){if(computerOrHuman[i]==1||boardWithSnake.snakes[i].dead){res.push(null)}
else{var board=boardWithSnake.board;var snake=boardWithSnake.snakes[i];var head=boardWithSnake.snakes[i].headToTail[0];var possibleMoves=possibleDirections();var validMoves=[];var foodMove=null;for(var j=0;j<possibleMoves.length;j++){var move=possibleMoves[j];if(!isValid(move,board,snake)){continue}
if(hasFoodThisWay(head,move,board)){foodMove=move;break}
validMoves.push(move)}
if(foodMove){res.push(foodMove)}
else{var maxScore=-1;var bestMove=null;for(var i_1=0;i_1<validMoves.length;i_1++){var move=validMoves[i_1];var score=scoreCell(board,head.row+move.shiftX,head.col+move.shiftY);if(score>=maxScore){maxScore=score;bestMove=move}}
res.push(bestMove)}}}
return res}
aiService.findComputerMove=findComputerMove;function possibleDirections(){var possibleMoves=[];possibleMoves.push({shiftX:1,shiftY:0},{shiftX:-1,shiftY:0},{shiftX:0,shiftY:1},{shiftX:0,shiftY:-1});return possibleMoves}
function hasFoodThisWay(head,direction,board){if(gameLogic.isBarrierOrBorderOrOpponentOrMySelf(head.row+direction.shiftX,head.col+direction.shiftY,board)){return!1}
if(board[head.row+direction.shiftX][head.col+direction.shiftY]==='FOOD'){return!0}
if(gameLogic.isBarrierOrBorderOrOpponentOrMySelf(head.row+2*direction.shiftX,head.col+2*direction.shiftY,board)){return!1}
if(board[head.row+2*direction.shiftX][head.col+2*direction.shiftY]==='FOOD'){return!0}
if(gameLogic.isBarrierOrBorderOrOpponentOrMySelf(head.row+3*direction.shiftX,head.col+3*direction.shiftY,board)){return!1}
if(board[head.row+3*direction.shiftX][head.col+3*direction.shiftY]==='FOOD'){return!0}}
function isValid(newDirection,board,snake){var head=snake.headToTail[0];var oldDirection=snake.currentDirection;return!gameLogic.isBarrierOrBorderOrOpponentOrMySelf(head.row+newDirection.shiftX,head.col+newDirection.shiftY,board)&&!(oldDirection.shiftX+newDirection.shiftX==0&&oldDirection.shiftY+newDirection.shiftY==0)}
function haveEnoughSpace(board,row,col){var hasMore=!0;var current=[];for(var i=0;i<board.length*board.length;i++){current.push(0)}
var N=board.length;current[encode(row,col,N)]=1;while(hasMore){hasMore=!1;for(var i=0;i<board.length*board.length;i++){if(current[i]==0){continue}
var thisRow=(i-i%N)/N;var thisCol=i%N;var directions=possibleDirections();for(var j=0;j<directions.length;j++){var direction=directions[j];if(!gameLogic.isBarrierOrBorderOrOpponentOrMySelf(thisRow+direction.shiftX,thisCol+direction.shiftY,board)){var code=encode(thisRow+direction.shiftX,thisCol+direction.shiftY,N);if(current[code]==0){current[code]=1;hasMore=!0}}}}
if(sizeOf(current)>=20){return!0}}
if(sizeOf(current)<20){return!1}
return!0}
function sizeOf(x){var count=0;for(var i=0;i<x.length;i++){if(x[i]==1){count++}}
return count}
function encode(row,col,N){return row*N+col}
function scoreCell(board,row,col){if(gameLogic.isBarrierOrBorderOrOpponentOrMySelf(row,col,board)){return-1}
if(!haveEnoughSpace(board,row,col)){return 0}
var score=0;for(var i=0;i<board.length;i++){for(var j=0;j<board.length;j++){if(i==row&&j==col){continue}
if(board[i][j]=='FOOD'){var gap=1.0/(Math.abs(i-row)+Math.abs(j-col));score=score+gap}}}
return score}})(aiService||(aiService={}))
